...
def reinitialize_weights(layer_key, reinitialized_layer_list):
    REINITIALIZE_WEIGHTS_FILE = "weights/reinitialize_weights_%s.bin"%layer_key
...
def main(args, reinitialize_weight_file):
    if (
...
if __name__ == "__main__":
    ...
    # loop over layers for reinitialization
    # weight_dict = OrderedDict()
    # for i in range(12):
    # # for i in [10]:
    ...
    current_date = get_date()
    save_result('outputs/allrun_results_layer10_attention_reinitialization_for_task_%s_%s.txt'%(args.task_name, current_date) , results)  
